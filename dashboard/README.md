...
- 🌗 **明暗主题切换**：极客代码面板风与清新白底风一键切换（持久化保存）。配色含默认 / Nord / Catppuccin / 抹茶 / **论文**（白底大字、学术图网格与 hatch 柱，固定亮色）。
- 📊 **核心 KPI 概览**：直观展示总等价金额 (USD)、总请求数、Token 总计以及缓存命中率。
- 📅 **自定义日期筛选**：选择指定时间段，所有图表与数据联动更新。
- 💰 **每日用量趋势图**：使用面积图直观展示每日等价 API 消耗金额的变化趋势。
- 📈 **模型分类栈图**：将用量拆分为 `API`、`First-party`（含 Composer / Grok 4.5 / 4.6）和 `Auto` 三大池子，支持在 USD 金额和 Tokens 数量间切换。
